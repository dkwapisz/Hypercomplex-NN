...
#         return tf.concat([tf.math.log1p(magnitude), phase_rg, phase_rb, magnitude * tf.cos(phase_rg + phase_rb)],
#                          axis=-1)
#     elif color_space == "HSV":
#         image = tf.image.rgb_to_hsv(image)
#         h, s, v = tf.split(image, 3, axis=-1)
...
#         phase = tf.atan2(q, i)
#         return tf.concat([y, magnitude * tf.cos(phase), magnitude * tf.sin(phase), tf.sin(y)], axis=-1)
#     elif color_space == "CMYK":
#         cmy = 1 - image
#         k = tf.reduce_min(cmy, axis=-1, keepdims=True)
#         denominator = 1 - k + 1e-8
